...
- **`start-task`** — Two modes: (A) in-place, write `.tasks/<slug>.md` and stay in the current dir; (B) worktree, create `.worktrees/<slug>/` on a clean `<type>/<slug>` branch and switch the **current** session into it via the native `EnterWorktree` tool — no second session, no manual `cd`, no CWD trap. Auto-delegates to `init-repo` if not in a git repo.
