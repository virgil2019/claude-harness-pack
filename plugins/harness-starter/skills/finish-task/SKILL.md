...
> Note on deferred items: as of v0.9.0, deferred / follow-up work is **not** collected here. start-task's behavioral rule says AI must propose creating a GitHub issue at the moment of deferral (and append the issue URL to `.task.md` Notes). If the workflow was followed, those issues are already tracked, and the PR body (auto-generated from `.task.md`) already lists them.
